...
func (r *PatientRepositoryPG) GetByPhoneHMAC(ctx context.Context, phoneHMAC string) (*domain.Patient, error) {
	q := `SELECT id, full_name, dob, gender, phone_encrypted, phone_hmac, email_encrypted, email_hmac, created_at, updated_at
	      FROM patients WHERE phone_hmac = $1`

	row := r.db.QueryRow(ctx, q, phoneHMAC)
	var p domain.Patient
	err := row.Scan(&p.ID, &p.FullName, &p.DOB, &p.Gender, &p.PhoneEncrypted, &p.PhoneHMAC, &p.EmailEncrypted, &p.EmailHMAC, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
