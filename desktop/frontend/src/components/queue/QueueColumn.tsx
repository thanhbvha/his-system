...
export const QueueColumn: React.FC<QueueColumnProps> = ({ serviceType, entries, onCallNext, onSkip }) => {
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 p-4 rounded-lg bg-blue-50 border border-blue-200 text-center relative overflow-hidden">
        ...
        {calledEntry ? (
          <div className="animate-pulse-slow">
            <div className="text-5xl font-black text-blue-600 mb-2">{calledEntry.queue_number}</div>
            <div className="text-lg font-medium text-gray-700">{calledEntry.patient.full_name}</div>
            <div className="mt-4 flex justify-center gap-2">
              <Button size="small" onClick={() => onSkip(calledEntry.id)} icon={<FastForwardOutlined />}>
                {t("queue.skip")}
              </Button>
            </div>
          </div>
      ...
      <Button 
        ...
        onClick={() => nextWaiting ? onCallNext(nextWaiting.id) : null}
          ...
          {waitingEntries.map(entry => (
            <div key={entry.id} className="flex justify-between items-center p-3 rounded-md bg-gray-50 hover:bg-gray-100 transition-colors">
              <div className="flex items-center gap-3">
                <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white shadow-sm border border-gray-200 font-bold text-gray-700">
                  {entry.queue_number}
                </span>
                <span className="font-medium text-gray-800">{entry.patient.full_name}</span>
              </div>
